...
void Time_Display(uint32_t TimeVar,struct rtc_time *tm)
{
	  ...
	  sprintf((char *)str,"%d",tm->tm_year);
		 //show year
	ILI9341_DispString_EN(102	,197,(char*)str);
		...
		sprintf((char *)str,"                        %s",en_WEEK_STR[tm->tm_wday]);
			  //show mon day
	ILI9341_DispString_EN(240,58,(char*)str);
				//日期
		sprintf((char *)str,"  %d-%d", 
																						tm->tm_mon,
																						tm->tm_mday);
	  //show wday
	ILI9341_DispString_EN(3,73,(char*)str);
...
//		//时间
  ...
  ILI9341_DrawCircle(120,172,90,0);
	LCD_SetTextColor(GREEN);
  ILI9341_DrawCircle(120,172,3,0);
	LCD_SetTextColor(GREEN);
  ILI9341_DrawCircle(35,83,30,0);
	LCD_SetTextColor(GREEN);
  ILI9341_DrawCircle(205,83,30,0);

	
	for (n = 0; n < 60; n++)
	{
		a = 120 + (85 * sin(n * 2 * PI / 60));
		b = 172 - (85 * cos(n * 2 * PI / 60));
		a1 = 120 +(80 * sin(n * 2 * PI / 60));
		b1 = 172 -(80 * cos(n * 2 * PI / 60));
		if (n % 5 == 0)
		{
			a = 120 + (75 * sin(n * 2 * PI / 60));
			b = 172 - (75* cos(n * 2 * PI / 60));
			a1 = 120 + (85 * sin(n * 2 * PI / 60));
			b1 = 172 - (85 * cos(n * 2 * PI / 60));
		}
...
sec1=tm->tm_sec;
	...
	x_sec = 120 + (double)(65 * sin(a_sec));///???????X??????
	y_sec = 172 - (double)(65 * cos(a_sec));///???????y??????
	x_min = 120 + (double)(55 * sin(a_min));///???????X??????
	y_min = 172 - (double)(55 * cos(a_min));///???????y??????
	x_hour = 120 +(double)(40 * sin(a_hour));///???????X??????
	y_hour = 172 -(double)(40 * cos(a_hour));///???????y??????
